//
//  TypesIndexTableViewController.m
//  Coupons
//
...
#import "TypesIndexTableViewController.h"
...
@interface TypesIndexTableViewController ()

@end

@implementation TypesIndexTableViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([arrayTypes count] == 0)
        [[[[self tabBarController] selectedViewController] tabBarItem] setBadgeValue:[NSString stringWithFormat:@"%d", [arrayTypes count]]];
    else
        [[[[self tabBarController] selectedViewController] tabBarItem] setBadgeValue:[NSString stringWithFormat:@"%d", [arrayTypes count]-1]];
}
        ...
        {            
            ...
            switch (status_code)
            {
                case 200:
                {
                    ...
                    [type setType_id:0];
                    [type setName:@"Όλοι οι τύποι"];
                    [type setOffer_count:[[result valueForKey:@"total_offers"] intValue]];
                    
                    [arrayTypes addObject:type];
                    
                    
                    for (NSDictionary *category in [result valueForKey:@"types"]){
                        
                        clsType *type = [[clsType alloc] init ];
                        
                        [type setType_id:[[category objectForKey:@"id"] intValue]];
                        [type setName:[[category objectForKey:@"name"] capitalizedString]];
...
{
    clsType *type = (clsType *) [arrayTypes objectAtIndex:indexPath.row];
...
{
    if ([[segue identifier] isEqualToString:@"ShowSelectedType"]) {
